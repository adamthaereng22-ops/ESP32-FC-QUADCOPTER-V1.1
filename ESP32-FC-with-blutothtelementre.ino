...
constexpr int ESC_M3_PIN = 26; // Rear Left   CW
constexpr int ESC_M4_PIN = 33; // Front Left  CCW
...
TinyGPSPlus gps;
BluetoothSerial SerialBT; // البلوتوث
...
void setup(){
  Serial.begin(115200);
  SerialBT.begin("ESP32_Quad"); // اسم البلوتوث على الهاتف
  delay(500);
...
void loop(){
  ...
  /******** Debug Print to Serial & Bluetooth ********/
...
if (millis() - lastPrint >= 80) {
    ...
    // إرسال البيانات لكل من Serial و Bluetooth إذا متصل
    ...
    Serial.print(yawAngle_deg,1);
    Serial.print(" u=");
    Serial.print((int)uRoll); Serial.print(",");
    Serial.print((int)uPitch); Serial.print(",");
    Serial.print((int)uYaw);
    Serial.print(" M=");
    Serial.print(m1); Serial.print(",");
    Serial.print(m2); Serial.print(",");
    Serial.print(m3); Serial.print(",");
    Serial.print(m4);
    Serial.print(" T="); Serial.print(bme_temp_C,1);
    Serial.print("C Alt="); Serial.print(bme_alt_m,1);
    Serial.print(" GPS_HOLD="); Serial.println(gpsHoldEnabled?"ON":"OFF");

    // إذا هناك جهاز بلوتوث متصل، أرسل نفس البيانات
    if (SerialBT.hasClient()) {
        SerialBT.print("ARM="); SerialBT.print(armed?"ON ":"OFF");
        SerialBT.print(" THR="); SerialBT.print(baseThrottle);
        SerialBT.print(" R/P/Y=");
        SerialBT.print(roll_deg,1); SerialBT.print("/");
        SerialBT.print(pitch_deg,1); SerialBT.print("/");
        SerialBT.print(yawAngle_deg,1);
        SerialBT.print(" u=");
        SerialBT.print((int)uRoll); SerialBT.print(",");
        SerialBT.print((int)uPitch); SerialBT.print(",");
        SerialBT.print((int)uYaw);
        SerialBT.print(" M=");
        SerialBT.print(m1); SerialBT.print(",");
        SerialBT.print(m2); SerialBT.print(",");
        SerialBT.print(m3); SerialBT.print(",");
        SerialBT.print(m4);
        SerialBT.print(" T="); SerialBT.print(bme_temp_C,1);
        SerialBT.print("C Alt="); SerialBT.print(bme_alt_m,1);
        SerialBT.print(" GPS_HOLD="); SerialBT.println(gpsHoldEnabled?"ON":"OFF");
    }
}}